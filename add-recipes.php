...
<h1 class="mt-3 adds__h1">Хотите, чтобы ваше блюдо стало популярным? Заполните форму и отправьте информацию о нем!</h1>
<form name="form_add" method="post" action="?p=add-recipes" class="mt-4 adds__form">
...
if( isset($_POST['button']) && $_POST['button']== 'Добавить блюдо')
{
    ...
    if( !$sql_res )
        echo '<h3 class="text-danger text-center adds_bad">Запись не добавлена</h3>';
    else 
        echo '<h3 class="text-success text-center adds_good">Запись добавлена</h3>';
    mysqli_close($mysqli);    
    }
